...
fn page_table_map(cspace: &CapNode, vspace: &mut PageTable, current: &TaskContext, vaddr: usize)
    -> ResultWithErr<MessageInfo> {
...
fn page_map(cspace: &CapNode, vspace: &mut PageTable, current: &TaskContext, vaddr: usize)
    -> ResultWithErr<MessageInfo> {
